 ...
 *
 * 布局：不用 el-card，改用自绘的 .panel。EP 卡片的头部内边距与边框在浅色页面里
 * 偏重，一页摞五张会显得很「后台系统」；自绘面板让标题、说明、操作按钮的层级更清晰。
 */
      ...
      <section class="panel">
        ...
        <div class="panel__body">
          ...
          <template v-else-if="healthData">
            ...
            <!-- 组件清单用行而不是描述列表：一行一组件，状态点在最左，扫读更快 -->
            <ul class="components">
              <li
              ...
              >
                <span
                  class="components__dot"
                  :class="item.ok ? 'components__dot--ok' : 'components__dot--bad'"
                />
                <span class="components__name">{{ COMPONENT_LABELS[name] ?? name }}</span>
                <span class="components__detail">{{ item.detail ?? '—' }}</span>
              </li>
            </ul>
          </template>
