...
8. **生成仍走抽句式降级路径，不是 LLM** —— 本机 `DEEPSEEK_API_KEY` 未配置。
...
| # | 现象 | 影响 | 处理 |
| --- | --- | --- | --- |
| 1 | chromadb 在本机报 `error returned from database: (code: 14) unable to open database file`（原生 sqlite 可写、`/tmp` 与工作区路径均失败） | 向量库降级为内置 simple | 代码已自动降级且统计里标注；**部署环境（Docker）需复验 chroma 可用性**，配置项 `VECTOR_BACKEND=chroma` 一处切换即可（store.py 是唯一出口） |
...
| 3 | 未配置 `DEEPSEEK_API_KEY` | 生成走抽句式降级（**仅生成环节**；向量化与精排已是本地模型，不依赖任何密钥） | 配密钥后自动走 LLM 路径；**接上后必须重跑 `scripts/eval.py`** 对比要点命中率 |
| 4 | 图片类样例（3 条）无 fixtures 图片 | 多模态评测无法执行 | 归平台侧：图片文件 + `ingest_image` 节点；`eval.py --images` 已预留入口 |
