...
> **接手这个项目的人请先看 [`接手任务清单.md`](接手任务清单.md)** ——
> 按顺序告诉你做什么、怎么算做完、哪些是红线。
> **要给别人演示/讲解**：操作步骤看 [`使用手册.md`](使用手册.md)，
> 讲稿与答辩问答看 [`讲解提纲（演示与答辩）.md`](讲解提纲（演示与答辩）.md)。
...
| G2 | 引用覆盖率 | **100%（不可降级）** | **100%**（空覆盖计数 `citation_coverage_vacuous` = **0**） |
| G2 | 引用真实性 | ≥ 95% | **100%**（270 项引用、0 伪造） |
| G3 | 要点命中率（严格口径 / 宽松口径） | ≥ 80% | **87.6%** / **100%**（185/185 要点；多次复跑落在 **84%~90%**；未配 LLM 的抽句式降级值为 44.7%） |
| G3 | 无依据拒答率 | ≥ 90% | **100%**（5/5 负样本） |
| G4 | 图片关键信息提取与作答 | ≥ 85% | **5/5 通过**（含 1 条按设计拒答的模糊纸条） |
| G5 | 文字 P95 / 带外部源 P95 | ≤ 8s / ≤ 15s | 空闲环境 **3.9s / 4.3s**；同机并行跑演示与子任务时 13.7s / 19.1s（最差一次 24.0s / 23.3s） |
| — | 误拒正样本 | 0 | **0 / 40** |
| — | MRR / NDCG@5 | — | 0.9750 / 0.9785 |
| — | 引用归属（LLM 路径） | — | 自动补引用 44 句 / 因无依据省略 27 句（省略会在拒答原因里说明） |

> 证据文件（已入库，可直接打开核对）：文本 `data/eval/report_20260920_184613.json`、
> 图片 `data/eval/report_image_20260920_184820.json`。
> **G3 与延迟必须按多次复跑的区间报告**（LLM 有随机性、延迟随机器负载变化），不要拿单次最好值当结论。

> 复跑方式见下方「评测与验收」。逐项证据见 `功能完成度清单.md`，交付清单见 `交付说明.md`。

**尚未完成**（详见 `接手任务清单.md` §2、§3）：三块新业务**前端页面的打磨**（页面骨架与接口已就绪）、
...
原开发文档 §1.3 列为「不做 / 后续批次」的三块功能，按项目方要求补齐**前后端**：
